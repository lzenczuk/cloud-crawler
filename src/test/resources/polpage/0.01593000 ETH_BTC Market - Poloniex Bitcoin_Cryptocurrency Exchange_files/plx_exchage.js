...
function restoreSettingsFromStorage(s) {
	...
	if (settings.marketRows != undefined) {
		$('#rowButtons').find('.show' + settings.marketRows).addClass('active');
	} else {
		$('#rowButtons').find('.show20').addClass('active');
	}

	if (settings.marketCols != undefined) {
		$('#colButtons').find('[data-url="' + settings.marketCols + '"]').addClass('active');
		//showMarketColumn('BTC', settings.btcCols, false); //fix a bug where the tables forget the right column, this should be saved by DataTables but it's being lost
	} else {
...
function refreshTradeHistory(){
    var tradeHistURL = '/public?command=returnTradeHistory&currencyPair=' + currencyPair;
...
function refreshOrderBook_br(){
	if (isLocal)
    	var url = 'https://poloniex.com/public?command=returnOrderBook&depth=9999999&currencyPair=' + currencyPair;
    else
    	var url = '/public?command=returnOrderBook&depth=9999999&currencyPair=' + currencyPair;
...
function refreshOrderBook_quick(){
	if (isLocal)
    	var url = 'https://poloniex.com/public?command=returnOrderBook&depth=' + orderbookDisplayLimit + '&currencyPair=' + currencyPair;
    else
    	var url = '/public?command=returnOrderBook&depth=' + orderbookDisplayLimit + '&currencyPair=' + currencyPair;
...
function drawFullMarket() {
	...
	orderBookReady = false;
	setBookUIstate();
    document.title = d.last + ' ' + d.pair + ' Market - Poloniex Bitcoin/Cryptocurrency Exchange';
...
function tickerEvent(args, kwargs) {
	...
	if(currencyPair.toLowerCase() === pair){
		...
		if (updateContents){
			...
			document.title = newLast + ' ' + secondaryCurrency + '/' + primaryCurrency + ' Market - Poloniex Bitcoin/Cryptocurrency Exchange';
		}
		if (updateChange){
			if(newPercentChange < 0)
...
function completelyResetWebsocket(){
	...
	if ('conn' in window){
		resetWebsocket();
	} else if (window.connection.session !== null){
		window.connection.close();
		initWebSockets(true);
	}
}
...
function checkHeartbeat(){
	if (updatesPaused)
		return;
	if ((Math.floor(Date.now()/1000) - lastHeartbeat) > (newWebSockets ? 4 : 10) || (Object.keys(marketEventQueue).length > 20 && !(seq+1 in marketEventQueue)))
		completelyResetWebsocket();
}
...
$("#sellForm").submit(function (event) {
    ...
    if (document.getElementById('sellAmount').value < 0.0001) {
        $("#result").empty().append("Amount must be greater than 0.0001.");
        showAlert();
        return;
    }
